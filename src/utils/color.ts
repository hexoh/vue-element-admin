...
const contrast = (rgb1: string[], rgb2: number[]) => {
  const [r1, g1, b1] = rgb1.map((v) => ~~v) as [number, number, number]
  const [r2, g2, b2] = rgb2 as [number, number, number]
  return (luminanace(r1, g1, b1) + 0.05) / (luminanace(r2, g2, b2) + 0.05)
}
...
export const calculateBestTextColor = (hexColor: string) => {
  const rgbColor = hexToRGB(hexColor.substring(1))
    .replace(/(?:\(|\)|rgb|RGB)*/g, '')
    .split(',')
  const contrastWithBlack = contrast(rgbColor, [0, 0, 0])
